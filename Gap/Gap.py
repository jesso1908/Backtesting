...
#     def next(self):
#         datahigh=self.data.high.get(size=self.swing_range)
#         datalow= self.data.low.get(size=self.swing_range)
#         if datahigh.pop(self.p.period) > max(datahigh):
#             # self.lines.swings[-self.p.period] = 1 #add new swing
#             self.lines.signal[0] = 1 #give a signal
#         elif datalow.pop(self.p.period) < min(datalow):
#             self.lines.swings[-self.p.period] = -1 #add new swing
#             self.lines.signal[0] = -1 #give a signal
#         else:
